...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  const webSearchEnv = (process.env.BRAVE_API_KEY ?? "").trim() || (process.env.PERPLEXITY_API_KEY ?? "").trim() || (process.env.OPENROUTER_API_KEY ?? "").trim();
    ...
    hasWebSearchKey
      ? [
        ...
        webSearchKey
          ...
          : "API key: provided via BRAVE_API_KEY/PERPLEXITY_API_KEY/OPENROUTER_API_KEY env var (Gateway environment).",
        "Docs: https://docs.molt.bot/tools/web",
      ].join("\n")
      : [
        "If you want your agent to be able to search the web, you’ll need an API key.",
        "",
        "Moltbot uses DuckDuckGo for the `web_search` tool (free).",
        "If you prefer Brave Search or Perplexity, you can configure them at any time.",
        "- Run `moltbot configure --section web` to set a custom search provider.",
        "Docs: https://docs.molt.bot/tools/web",
      ].join("\n"),
    "Web search (optional)",
  );
