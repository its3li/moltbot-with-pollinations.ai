...
async function promptWebToolsConfig(
  ...
  note(
    [
      ...
      "It uses DuckDuckGo by default (free, no API key required).",
  ...
  const enableSearch = guardCancel(
    await confirm({
      message: "Enable web_search (DuckDuckGo)?",
  ...
  if (enableSearch) {
    const keyInput = guardCancel(
      await text({
        message: hasSearchKey
          ? "You can leave this blank for DuckDuckGo (to keep current key, or use BRAVE_API_KEY)."
          : "You can leave this blank for DuckDuckGo (or paste a Brave Search API key here).",
    ...
    if (key) {
      ...
      note(
        [
          "No key stored yet, so web_search will use DuckDuckGo.",
          "If you want to use Brave Search, store a key here or set BRAVE_API_KEY in the Gateway environment.",
          "Docs: https://docs.molt.bot/tools/web",
        ].join("\n"),
        "Web search",
      );
