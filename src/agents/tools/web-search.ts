...
const SEARCH_PROVIDERS = ["duckduckgo", "perplexity"] as const;
...
// Freshness is not supported by DuckDuckGo HTML scraping.
function normalizeFreshness(_value: string | undefined): string | undefined {
  return undefined;
}
...
async function runWebSearch(params: {
  ...
  const res = await fetch(url.toString(), {
    method: "POST", // DuckDuckGo HTML likes POST for searches
    headers: {
  ...
  // This is a bit fragile but works for the basic free version
  const resultRegex =
    /<div class="result__body">[\s\S]*?<a class="result__a" href="([\s\S]*?)">([\s\S]*?)<\/a>[\s\S]*?<a class="result__snippet"[\s\S]*?>([\s\S]*?)<\/a>/g;
  let match;
  while ((match = resultRegex.exec(html)) !== null && results.length < params.count) {
    ...
    results.push({
      ...
      url: finalUrl,
      description: rawSnippet.replace(/<[^>]*>?/gm, "").trim(),
    });
  }
...
export function createWebSearchTool(options?: {
  ...
  const description =
    provider === "perplexity"
      ...
      : "Search the web using DuckDuckGo. Returns titles, URLs, and snippets for research (free, no API key required).";
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (rawFreshness && provider !== "duckduckgo") {
        return jsonResult({
          error: "unsupported_freshness",
          message: "freshness is not supported by the DuckDuckGo web_search provider.",
          docs: "https://docs.molt.bot/tools/web",
        });
      }
